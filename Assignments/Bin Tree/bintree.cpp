...
// Jan 26, 2016
// ----------------------------------------------------------------------------
// Purpose - The purpose of this program is to store and access NodeDatas
...
// program also stores each NodeData inside a Node that will be place in the
// tree.
...
// ------------------------- retrieve ---------------------------------
// Description: This method will retrieve a NodeData from a node inside the
// tree. It uses a constant NodeData to find the NodeData inside the tree. It
// calls a helper method to find the Node
// Preconditions: none
// Postconditions: tree is not changed. returns a boolean if Nodedata was found
// ----------------------------------------------------------------------------
bool BinTree::retrieve(const NodeData& target, NodeData*& retrieved) const {
    return retrieveHelper(root, target, retrieved); //calls helper method
                                                    // with tree root, target,
                                                    // and storage NodeData
}
// ------------------------- retrieveHelpet ---------------------------------
...
// ----------------------------------------------------------------------------
bool BinTree::retrieveHelper(const Node *subtree, const NodeData& target,
...
// ------------------------- insert ---------------------------------
// Description: This method will insert a NodeData into the tree. It uses a
...
// ----------------------------------------------------------------------------
bool BinTree::insertHelper(Node*& subtree, NodeData* inserted) {
    if(subtree == nullptr ||
...
// ------------------------- getHeight ---------------------------------
// Description: This method will find a NodeData inside the tree and return
// its relative height including itself. If the NodeData if the leaf, it will
// return one, if it is the parent to a leaf, it will return 2 and so on. If
// the NodeData is not in the tree, it will return 0. This method calls two
...
// Preconditions: a instance of a bintree and a NodeData
...
// ------------------------- getHeightNodeHelper ------------------------------
// Description: This helper method will find a NodeData inside the tree and
// call the second helper method to get the height of that node. If the
// NodeData does not exist in the tree, it will return 0 from here. This
// method recurse through the nodes assuming this is not a BSTree.
// Preconditions: a root/subtree. and a target NodeData
...
void BinTree::preOrderCopy(Node* copySubtree, Node* &subtree) {
    if(copySubtree != nullptr){     //case: node of the copy tree is not null
        ...
        NodeData* copyData =
                new NodeData(*copySubtree->data); //make a copy of the other
                                                  // NodeData
...
// ------------------------- arrayToBSTree ---------------------------------
// Description: This method will convert an array of NodeData to a BinTree.
...
// Preconditions: an array of NodeData
...
// the array.
// Preconditions: an array of NodeData, a low, and a high
// Postconditions: new bintree has been built and has elements inside
// ----------------------------------------------------------------------------
void BinTree::arrayToBSTreeHelper(NodeData* array[],int low, int high){
    if(low>high){                   //case: time to end the recursion
        ...
        NodeData* temp = array[mid];//create a pointer to value in array
...
// ----------------------------------------------------------------------------
int BinTree::bstreeToArrayHelper(Node *subtree, NodeData * array[]) {
    if(subtree == nullptr){     //case: node is null, go back up to add element
        ...
        NodeData* temp = subtree->data; //create a temp NodeData to the tree
                                        // data
...
//------------------------------- buildTree ----------------------------------
// This method will build a tree from a line of strings read in through the
// input stream.
...
void buildTree(BinTree& T, ifstream& infile) {
    ...
    for (;;) {
        ...
        NodeData* ptr = new NodeData(s);     // NodeData constructor takes
                                             // string
...
//Precondtion: array not set to nulls
//post condition: array elements set to null
