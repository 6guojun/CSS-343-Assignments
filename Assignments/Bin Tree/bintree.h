...
public:
    ...
    //returns true
    bool insert(NodeData*);                 //method used to insert a
                                            // NodeData into the tree
    bool retrieve(const NodeData&, NodeData*&) const; //method to retrieve a
                                            // NodeData from the tree
    int getHeight(const NodeData&) const;   //method to get the height of a
                                            // NodeData
    void bstreeToArray(NodeData* []);       //Method to convert a tree into
                                            // an array
    void arrayToBSTree(NodeData* []);       //method to convert an array to a
                                            // tree
    void displaySideways() const;			// display a tree sideways

private:
    //the Node for storing a NodeData in the tree
    ...
    //helper methods
    bool retrieveHelper(const Node*, const NodeData&,
                        NodeData* &) const; //helper to retrieve the NodeData
    bool insertHelper(Node*&, NodeData*);   //helper to insert a NodeData
    int getHeightHelper(const Node*) const; //helper to get the height
    int getHeightNodeHelper(const Node*,
                            const NodeData&) const; //helper to get the node
                                                    // for height
    ...
    void arrayToBSTreeHelper(NodeData* [], int, int); //helper to convert
                                                      // array to tree
    int bstreeToArrayHelper(Node* subtree, NodeData* []); //helper to convert
                                                    // tree to an array
};                                          //close class brace
